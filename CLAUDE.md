...
- **Position opened date**: Now tracked via `positions` table. Each position lifecycle gets a unique `position_id`. When position closes and reopens, a new `positions` record is created with fresh `opened_at`. The `position_snapshots.position_id` links snapshots to their position lifecycle. `get_open_positions()` JOINs to `positions` table for authoritative `opened_at`.
- **Position ID partial closes (UNTESTED)**: Position ID stays same during partial closes. This may cause unexpected behavior if position is partially closed then re-added to. Needs real-world testing. If issues occur, consider adding sub-IDs (e.g., "2a", "2b") for partial close tracking.
